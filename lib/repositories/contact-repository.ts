...
export class ContactRepository {
    ...
    async update(
        ...
        if (!updated) {
            throw new Error(
                `Contact ${contactId} could not be found after update`,
            );
        }
    ...
    async removeFromOpportunity(
        ...
        const association =
            await this.db.opportunityContact.findFirst({
                where: {
                    opportunityId,
                    contactId,
                    opportunity: {
    ...
    async getForOpportunity(
        ...
        const opportunity =
            await this.db.opportunity.findFirst({
                where: {
                    id: opportunityId,
                    ownerId,
                },
        ...
        const associations =
            await this.db.opportunityContact.findMany({
                where: {
                    opportunityId,
                    opportunity: {
                        ownerId,
