...
describe("SubmissionService", () => {
  ...
  it("preserves the selected CV and cover-letter Artefact references", async () => {
    ...
    expect(result.coverLetterArtefactId).toBe("cover-letter-1");
  });
  ...
  it("gets the Submission for an Opportunity", async () => {
    ...
    const result = await service.getByOpportunityId(ownerA, "opportunity-1");
  ...
  it("propagates rejection when a Submission already exists", async () => {
    repository.create.mockRejectedValue(
      new Error("A Submission already exists for Opportunity opportunity-1"),
    );

    await expect(
      service.create(ownerA, "opportunity-1", {
        submittedAt: new Date("2026-08-19T10:00:00.000Z"),
        method: "PORTAL",
        cvArtefactId: "cv-1",
        coverLetterArtefactId: null,
  ...
  it("passes the owner scope to every repository operation", async () => {
    ...
    await service.getByOpportunityId(ownerB, "opportunity-1");
