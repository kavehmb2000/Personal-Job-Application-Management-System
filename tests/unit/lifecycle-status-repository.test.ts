...
describe("LifecycleStatusRepository", () => {
    ...
    beforeEach(async () => {
        const [owner, otherOwner] = await Promise.all([
            createOwner(
                `lifecycle-status-a-${crypto.randomUUID()}@example.com`,
            ),
            createOwner(
                `lifecycle-status-b-${crypto.randomUUID()}@example.com`,
            ),
        ]);
    ...
    it("gets an active lifecycle status by key within owner scope", async () => {
        const status = await repository.getByKey(
            ownerId,
            "DISCOVERED",
        );
    ...
    it("does not expose another owner's statuses when listing", async () => {
        ...
        expect(
            statuses.every((status) => status.ownerId === ownerId),
        ).toBe(true);
    });
});