...
- **FR-011**: The system MUST record a submission separately from the opportunity and retain submission date/time, CV version used, optional cover-letter version used, submission method, and submission notes; a reapplication or resubmission MUST be created as a separate opportunity record.
...
- **FR-015**: The system MUST support application-associated documents with name, type, version, file or external link, optional description, optional status, and application association.
...
- **FR-036**: Previously accessed critical application information and frequently used documents SHOULD remain readable during temporary connectivity loss where practical; the MVP does not require full offline synchronization.
- **FR-037**: The user MUST be able to export their structured application data and receive a practical record of important documents or document references, so the application is not the sole irreplaceable copy of career information.
...
- **Submission**: The dated record of applying for an opportunity, including the immutable identities of submitted asset versions and submission details.
...
- **SC-007**: A complete export makes 100% of structured application records readable independently of the application and identifies every referenced document and any document that could not be included.
...
- Export includes structured records plus document files where available and otherwise durable file references and metadata.
