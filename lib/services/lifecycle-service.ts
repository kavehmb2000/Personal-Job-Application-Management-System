...
export class LifecycleService {
  ...
  async validateTransition(
    ...
    const transition = await this.prisma.lifecycleTransition.findUnique({
      where: {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      await tx.opportunityEvent.create({
        data: {
          ...
          type: eventType,
          title: event.title ?? `${fromStatus.label} → ${toStatus.label}`,
          descriptionMarkdown: event.descriptionMarkdown ?? null,
