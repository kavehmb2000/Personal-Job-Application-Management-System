...
describe("UserActionRepository", () => {
  ...
  it("creates a UserAction within the Opportunity owner scope", async () => {
    const action = await repository.create(ownerA, opportunityA.id, {
  ...
  it("gets a UserAction within the owner's Opportunity scope", async () => {
    ...
    const result = await repository.getById(ownerA, opportunityA.id, action.id);
  ...
  it("does not expose another owner's UserAction", async () => {
    ...
    const result = await repository.getById(ownerB, opportunityA.id, action.id);
  ...
  it("rejects a stale-version update", async () => {
    ...
    await repository.update(ownerA, opportunityA.id, action.id, 1, {
      title: "First update",
    });

    await expect(
      repository.update(ownerA, opportunityA.id, action.id, 1, {
        title: "Stale update",
      }),
    ).rejects.toThrow(
