...
export async function GET(_request: Request, context: RouteContext) {
  try {
    ...
    const contact = await createService().getById(owner.id, id);
...
export async function PATCH(request: Request, context: RouteContext) {
  try {
    ...
    const input = await validateJsonRequest(updateContactSchema, request);

    const contact = await createService().update(owner.id, id, input);
