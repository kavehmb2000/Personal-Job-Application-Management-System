...
describe("ContactRepository", () => {
    ...
    it("associates a Contact with an Opportunity owned by the same owner", async () => {
        ...
        const association =
            await repository.addToOpportunity(
                ownerA,
                opportunityA.id,
                contact.id,
            );

        expect(association.opportunityId).toBe(
            opportunityA.id,
        );
        expect(association.contactId).toBe(
            contact.id,
        );
    });
    ...
    it("gets Contacts associated with an Opportunity", async () => {
        ...
        const contacts =
            await repository.getForOpportunity(
                ownerA,
                opportunityA.id,
            );
    ...
    it("removes an OpportunityContact association", async () => {
        ...
        await repository.removeFromOpportunity(
            ownerA,
            opportunityA.id,
            contact.id,
        );
    ...
    it("rejects removing an association outside the owner's scope", async () => {
        ...
        await expect(
            repository.removeFromOpportunity(
                ownerB,
                opportunityA.id,
                contact.id,
            ),
