...
const validTransitions: Array<[LifecycleStateKey, LifecycleStateKey]> = [
...
function makePrismaMock() {
  const statuses = new Map(
    (Object.keys(statusIds) as LifecycleStateKey[]).map((key) => [
      key,
      makeStatus(key),
    ]),
  );
  ...
  const validTransitionKeys = new Set(
    validTransitions.map(([from, to]) => `${from}->${to}`),
  );
  ...
  return {
    ...
    lifecycleTransition: {
      findUnique: vi.fn(async ({ where }: any) => {
        const from = (
          Object.entries(statusIds) as Array<[LifecycleStateKey, string]>
        ).find(
          ([, id]) => id === where.fromStatusId_toStatusId.fromStatusId,
        )?.[0];

        const to = (
          Object.entries(statusIds) as Array<[LifecycleStateKey, string]>
        ).find(
          ([, id]) => id === where.fromStatusId_toStatusId.toStatusId,
        )?.[0];

        if (from && to && validTransitionKeys.has(`${from}->${to}`)) {
          return {
            id: `${from}-${to}`,
...
describe("LifecycleService", () => {
  ...
  beforeEach(() => {
    ...
    service = new LifecycleService(prisma as never);
  });

  describe("validateTransition", () => {
    it.each(validTransitions)("accepts %s -> %s", async (from, to) => {
      await expect(
        service.validateTransition(ownerId, from, to),
    ...
    it.each([
      ...
      async (from, to) => {
        await expect(
          service.validateTransition(ownerId, from, to),
        ).rejects.toBeInstanceOf(InvalidOpportunityLifecycleTransitionError);
      },
    );

    it("rejects an inactive or missing source status", async () => {
      prisma.lifecycleStatus.findFirst.mockResolvedValueOnce(null);

      await expect(
        service.validateTransition(ownerId, "DISCOVERED", "SUBMITTED"),
      ).rejects.toThrow("Lifecycle status DISCOVERED was not found");
    });
    ...
    it("rejects an inactive or missing target status", async () => {
      ...
      await expect(
        service.validateTransition(ownerId, "DISCOVERED", "SUBMITTED"),
      ).rejects.toThrow("Lifecycle status SUBMITTED was not found");
    });
  });
});
