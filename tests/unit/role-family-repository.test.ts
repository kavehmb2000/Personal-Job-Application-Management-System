...
describe("RoleFamilyRepository", () => {
    ...
    beforeEach(async () => {
        const [owner, otherOwner] = await Promise.all([
            createOwner(
                `role-family-a-${crypto.randomUUID()}@example.com`,
            ),
            createOwner(
                `role-family-b-${crypto.randomUUID()}@example.com`,
            ),
        ]);
    ...
    it("gets a role family within the owner's scope", async () => {
        const roleFamily = await repository.getById(
            ownerId,
            roleFamilyId,
        );
    ...
    it("does not return inactive role families", async () => {
        ...
        expect(
            roleFamilies.some(
                (roleFamily) => roleFamily.name === "Inactive Family",
            ),
        ).toBe(false);
    });
    ...
    it("does not return another owner's role families", async () => {
        ...
        expect(
            roleFamilies.every(
                (roleFamily) => roleFamily.ownerId === ownerId,
            ),
        ).toBe(true);
    });
});