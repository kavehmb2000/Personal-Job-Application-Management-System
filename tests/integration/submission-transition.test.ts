...
describe("SubmissionService formal submission", () => {
    ...
    it("creates a Submission and changes Discovered to Submitted atomically", async () => {
        ...
        expect(result.opportunity.status.key).toBe(
            "SUBMITTED",
        );

        expect(result.submission.opportunityId).toBe(
            opportunityId,
        );

        expect(result.submission.submittedAt).toEqual(
            submittedAt,
        );
        ...
        const events =
            await prisma.opportunityEvent.findMany({
                where: { opportunityId },
            });
    ...
    it("rolls back the Submission when event creation fails", async () => {
        ...
        expect(opportunity.status.key).toBe(
            "DISCOVERED",
        );
    ...
    it("represents a reapplication with a new Opportunity rather than a second Submission", async () => {
        ...
        expect(firstSubmission.submission.opportunityId).toBe(
            firstOpportunity.id,
        );
    ...
    it("supports reapplication through a new Opportunity", async () => {
        ...
        expect(firstSubmission.opportunity.status.key).toBe(
            "SUBMITTED",
        );

        expect(secondSubmission.opportunity.status.key).toBe(
            "SUBMITTED",
        );
