...
export class CommunicationRepository {
  ...
  async removeArtefact(
    ...
    const association = await this.prisma.communicationArtefact.findFirst({
      where: {
        communicationId,
        artefactId,
      },
    ...
    if (!association) {
      throw new Error("CommunicationArtefact association could not be found");
    }
  ...
  private async requireArtefactOwner(ownerId: string, artefactId: string) {
    ...
    if (!artefact) {
      throw new Error("Artefact could not be found within the owner's scope");
    }
