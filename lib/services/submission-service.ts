...
export class SubmissionService {
    ...
    async getByOpportunityId(
        ...
        return this.repository.getByOpportunityId(
            ownerId,
            opportunityId,
        );
    }
    ...
    async submit(
        ...
        return this.db.$transaction(async (tx) => {
            ...
            const discoveredStatus =
                await tx.lifecycleStatus.findFirst({
                    where: {
                        ownerId,
                        key: "DISCOVERED",
                        isActive: true,
            ...
            if (!discoveredStatus || !submittedStatus) {
                throw new Error(
                    "Required lifecycle status was not found",
                );
            }
            ...
            const submission =
                await tx.submission.create({
                    data: {
                        ...
                        cvArtefactId:
                            input.cvArtefactId ?? null,
                        coverLetterArtefactId:
                            input.coverLetterArtefactId ?? null,
                    },
                });
